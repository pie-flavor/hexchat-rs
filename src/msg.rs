...
use crate::{Context, PrintEvent, to_cstring, from_cstring, c, ChannelRef};
...
impl Context {
    ...
    pub fn print_plain(&self, text: &str) {
        ...
        unsafe { c::hexchat_print(self.handle, text.as_ptr()); }
    }
        ...
        let res = unsafe {
            match arguments {
                ...
                [arg_1] => {
                    ...
                    c::hexchat_emit_print(self.handle, event.as_ptr(), arg_1.as_ptr(),
                        ptr::null::<()>())
                },
                [arg_1, arg_2] => {
                    let (arg_1, arg_2) = (to_cstring(arg_1.as_ref()), to_cstring(arg_2.as_ref()));
                    c::hexchat_emit_print(self.handle, event.as_ptr(), arg_1.as_ptr(),
                        arg_2.as_ptr(), ptr::null::<()>())
                },
                [arg_1, arg_2, arg_3] => {
                    let (arg_1, arg_2, arg_3) = (to_cstring(arg_1.as_ref()), to_cstring(arg_2.as_ref()),
                        to_cstring(arg_3.as_ref()));
                    c::hexchat_emit_print(self.handle, event.as_ptr(), arg_1.as_ptr(),
                        arg_2.as_ptr(), arg_3.as_ptr(), ptr::null::<()>())
                },
                [arg_1, arg_2, arg_3, arg_4, ..] => {
                    let (arg_1, arg_2, arg_3, arg_4) = (to_cstring(arg_1.as_ref()),
                        to_cstring(arg_2.as_ref()), to_cstring(arg_3.as_ref()),
                        to_cstring(arg_4.as_ref()));
                    c::hexchat_emit_print(self.handle, event.as_ptr(), arg_1.as_ptr(),
                        arg_2.as_ptr(), arg_3.as_ptr(), arg_4.as_ptr(), ptr::null::<()>())
                },
            }
        };
        res != 0
    ...
    /// or not it succeeded.
    pub fn print_event_at(&self, event: PrintEvent, timestamp: &DateTime<impl TimeZone>,
        arguments: &[impl AsRef<str>]) -> bool
    {
        unsafe {
            ...
            let res = match arguments {
                [] => {
                    c::hexchat_emit_print_attrs(self.handle, event_attrs, event.as_ptr(),
                        ptr::null::<()>())
                },
                [arg_1] => {
                    let arg_1 = to_cstring(arg_1.as_ref());
                    c::hexchat_emit_print_attrs(self.handle, event_attrs, event.as_ptr(),
                        arg_1.as_ptr(), ptr::null::<()>())
                },
                [arg_1, arg_2] => {
                    let (arg_1, arg_2) = (to_cstring(arg_1.as_ref()), to_cstring(arg_2.as_ref()));
                    c::hexchat_emit_print_attrs(self.handle, event_attrs, event.as_ptr(),
                        arg_1.as_ptr(), arg_2.as_ptr(), ptr::null::<()>())
                },
                [arg_1, arg_2, arg_3] => {
                    let (arg_1, arg_2, arg_3) =
                        (to_cstring(arg_1.as_ref()), to_cstring(arg_2.as_ref()), to_cstring(arg_3.as_ref()));
                    c::hexchat_emit_print_attrs(self.handle, event_attrs, event.as_ptr(),
                        arg_1.as_ptr(), arg_2.as_ptr(), arg_3.as_ptr(), ptr::null::<()>())
                },
                [arg_1, arg_2, arg_3, arg_4, ..] => {
                    let (arg_1, arg_2, arg_3, arg_4) =
                        (to_cstring(arg_1.as_ref()), to_cstring(arg_2.as_ref()),
                            to_cstring(arg_3.as_ref()), to_cstring(arg_4.as_ref()));
                    c::hexchat_emit_print_attrs(self.handle, event_attrs, event.as_ptr(),
                        arg_1.as_ptr(), arg_2.as_ptr(), arg_3.as_ptr(), arg_4.as_ptr(), ptr::null::<()>())
                },
            };
    ...
    /// succeeded.
    pub fn print_event_to_channel(&self, channel: &ChannelRef, event: PrintEvent,
        args: &[impl AsRef<str>]) -> bool
    {
        unsafe {
            ...
            if c::hexchat_set_context(self.handle, ctx) == 0 {
                c::hexchat_set_context(self.handle,
    ...
    pub fn print_event_to_channel_at(&self, channel: &ChannelRef, event: PrintEvent,
        timestamp: &DateTime<impl TimeZone>, args: &[impl AsRef<str>]) -> bool
    {
        unsafe {
            let ctx = c::hexchat_get_context(self.handle);
            if c::hexchat_set_context(self.handle, channel.handle) == 0 {
    ...
    pub fn add_modes(&self, targets: &[impl AsRef<str>], mode: char) -> bool {
        ...
        let len = targets.len();
        let ptrs = targets.iter().map(|x| to_cstring(x.as_ref()).into_raw()).collect::<Box<[_]>>();
        let ptr_ptr = Box::into_raw(ptrs);
        unsafe {
            c::hexchat_send_modes(self.handle, ptr_ptr as _, len as _, 0,
                mem::transmute(b'+'), mem::transmute(mode as u8));
            let ptrs = Box::from_raw(ptr_ptr);
            ptrs.into_vec().into_iter().for_each(|x| { CString::from_raw(x); });
        }
        true
    }
    ...
    pub fn remove_modes(&self, targets: &[impl AsRef<str>], mode: char) -> bool {
        ...
        let ptr_ptr = Box::into_raw(ptrs);
        unsafe {
            c::hexchat_send_modes(self.handle, ptr_ptr as _, targets.len() as _, 0,
                mem::transmute(b'-'), mem::transmute(mode as u8));
            let ptrs = Box::from_raw(ptr_ptr);
            ptrs.into_vec().into_iter().for_each(|x| { CString::from_raw(x); });
        }
    ...
    /// it succeeded.
    pub fn add_modes_in_channel(&self, targets: &[impl AsRef<str>], mode: char,
        channel: &ChannelRef) -> bool
    {
        unsafe {
            let ctx = c::hexchat_get_context(self.handle);
            if c::hexchat_set_context(self.handle, channel.handle) == 0 {
    ...
    /// not it succeeded.
    pub fn remove_modes_in_channel(&self, targets: &[impl AsRef<str>], mode: char,
        channel: &ChannelRef) -> bool
    {
        unsafe {
            let ctx = c::hexchat_get_context(self.handle);
    ...
    fn strip(&self, string: &str, mode: i32) -> Result<String, ()> {
        let stripped = unsafe {
            c::hexchat_strip(self.handle, string.as_bytes() as *const [u8] as _, string.len() as _,
                mode)
        };
        if stripped.is_null() {
            Err(())
            ...
            unsafe { c::hexchat_free(self.handle, stripped as _); }
            Ok(res)
        }
    }
