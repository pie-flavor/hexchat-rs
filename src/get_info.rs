use crate::{c, Context, from_cstring, to_cstring, from_cstring_opt};
...
impl Context {
    ...
    pub fn get_away_reason(&self) -> Option<String> {
        unsafe {
            from_cstring_opt(c::hexchat_get_info(self.handle, AWAY.as_ptr()))
        }
    }
    /// Gets whether or not the client is currently away.
    pub fn is_away(&self) -> bool {
        unsafe {
            c::hexchat_get_info(self.handle, AWAY.as_ptr()).is_null()
        }
    }
    /// Gets the name of the current channel.
    pub fn get_channel_name(&self) -> String {
        unsafe {
            from_cstring(c::hexchat_get_info(self.handle, CHANNEL.as_ptr()))
        }
    }
    ...
    pub fn get_charset(&self) -> Charset {
        unsafe {
            Charset::from_str(
                &CStr::from_ptr(
                    c::hexchat_get_info(self.handle, CHARSET.as_ptr()))
                    .to_string_lossy())
                .unwrap()
        }
    }
    ...
    pub fn get_config_dir(&self) -> PathBuf {
        unsafe {
            PathBuf::from(&*CStr::from_ptr(c::hexchat_get_info(self.handle, CONFIG_DIR.as_ptr()))
                .to_string_lossy())
        }
    }
